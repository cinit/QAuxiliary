...
import io.github.qauxv.util.dexkit.FriendDeleteCallbackClass;
...
public class DeletionObserver extends BasePersistBackgroundHook {
    ...
    private final XC_MethodHook exfriendEntryHook = new XC_MethodHook(55) {
        ...
        protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
            try {
                ...
                } catch (NullPointerException | NoSuchMethodError | IllegalArgumentException e) {
    ...
    private void hookDelFriendNt() {
        try {
            ...
            // 兼容性降级（目标方法签名变化等）不记为功能错误
            Log.i("QAuxv-Del: " + "hookDelFriendHandler(4参) 注册跳过: " + e);
        }
        // 新路径（9.3.60+ NT）：DelFriendServiceImpl.delFriend(String source, String uin, byte, int)
        try {
            findAndHookMethod(load("com.tencent.mobileqq.addfriend.api.impl.DelFriendServiceImpl"),
                "delFriend", String.class, String.class, byte.class, int.class, new XC_MethodHook(200) {
                    @Override
                    protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
                        try {
                            ...
                            try {
                                ...
                                Log.i("QAuxv-Del: " + "DelFriendServiceImpl uin 解析失败: " + uinStr);
                            }
                            if (deluin > 10000) {
                                long uin = AppRuntimeHelper.getLongAccountUin();
                                Log.i("QAuxv-Del: " + "主动删除请求(delservice): deluin=" + deluin);
                                ExfriendManager.get(uin).markActiveDelete(deluin);
                            }
                        } catch (Throwable e) {
                            Log.i("QAuxv-Del: " + "hookDelFriendServiceImpl 异常: " + e);
                        }
                    }
                });
            Log.i("QAuxv-Del: " + "hookDelFriendServiceImpl 注册成功");
        } catch (Throwable e) {
            // 兼容性降级不记为功能错误
            Log.i("QAuxv-Del: " + "hookDelFriendServiceImpl 注册跳过: " + e);
        }
        // 删除回调类名随版本变化，优先用 DexKit 特征定位，未命中时按已知类名探测
        Class<?> cbCls = null;
        String source = "DexKit";
        try {
            cbCls = DexKit.loadClassFromCache(FriendDeleteCallbackClass.INSTANCE);
            if (cbCls != null && !hasDelFriendCallback(cbCls)) {
                Log.i("QAuxv-Del: DexKit 缓存类无 onUpdateDelFriend，改用探测");
                cbCls = null;
            }
        } catch (Throwable e) {
            Log.i("QAuxv-Del: DexKit 缓存读取失败: " + e);
        }
        if (cbCls == null) {
            cbCls = findDelFriendCallbackClass();
            source = "探测";
        }
        if (cbCls == null) {
            Log.i("QAuxv-Del: 未找到 onUpdateDelFriend 删除回调，被动回调跳过");
            return;
        }
        Log.i("QAuxv-Del: 删除回调类(" + source + ")=" + cbCls.getName());
        hookOnUpdateDelFriend(cbCls);
    }

    private void hookOnUpdateDelFriend(Class<?> cbCls) {
        try {
            findAndHookMethod(cbCls,
            ...
            // 兼容性降级（目标方法缺失等）不记为功能错误
            Log.i("QAuxv-Del: " + "hookDelFriendNt(回调) 注册跳过: " + e);
        }
    }

    /**
     * DexKit 查找需在 deobfuscation section 内完成，查找成功后返回 false 才会正常初始化
     */
    @Override
    public boolean isPreparationRequired() {
        try {
            return DexKit.isRunDexDeobfuscationRequired(FriendDeleteCallbackClass.INSTANCE);
        } catch (Throwable e) {
            return false;
        }
    }
    ...
    public Step[] makePreparationSteps() {
        return new Step[]{new DexDeobfStep(FriendDeleteCallbackClass.INSTANCE)};
    }

    private boolean hasDelFriendCallback(Class<?> cls) {
        for (java.lang.reflect.Method mm : cls.getDeclaredMethods()) {
            if ("onUpdateDelFriend".equals(mm.getName()) && mm.getParameterCount() == 2) {
                return true;
            }
        }
        return false;
    ...
    private Class<?> findDelFriendCallbackClass() {
        for (String clsName : new String[]{
                "com.tencent.mobileqq.app.bb",
                "com.tencent.mobileqq.app.bc",
                "com.tencent.mobileqq.friend.observer.a"}) {
            Class<?> c = load(clsName);
            if (c != null && hasDelFriendCallback(c)) {
                return c;
            }
        }
        return null;
    ...
    private void hookGatheredContactsResp() {
        try {
            ...
            // 兼容性降级（目标方法缺失等）不记为功能错误
            Log.i("QAuxv-Del: " + "hookGatheredContactsResp 注册跳过: " + e);
        }
    }
    ...
    private void hookSplashActivity() {
        try {
            ...
            // 兼容性降级不记为功能错误
            Log.i("QAuxv-Del: " + "hookSplashActivity 注册跳过: " + e);
        }
    }
    ...
    private void hookFriendListEntry() {
        try {
            ...
            // 兼容性降级不记为功能错误
            Log.i("QAuxv-Del: " + "hookFriendListEntry 注册跳过: " + e);
        }
    }
}
