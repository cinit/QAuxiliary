...
object MiniAppDirectJump : CommonSwitchFunctionHook("MiniAppDirectJump::BaseChatPie", arrayOf(CArkAppItemBubbleBuilder, AbstractQQCustomMenuItem)),
    ...
    private fun toBiliApp(ctx: Activity, text: String) {
        ...
        if (matchResult != null) {
            val b23url = matchResult
            //Toasts.info(ctx, b23url)
            try {
                // 还原短链
                val expandedUrl = expandShortUrl(b23url)
                if (expandedUrl != null) {
                    //copyToClipboard(ctx, expandedUrl)
                    val regex = """https://www\.bilibili\.com/video/(BV\w+)\?""".toRegex()

                    // 查找匹配
                    val matchBV = regex.find(expandedUrl)
                    val bvNumber = matchBV?.groupValues?.get(1)
                    if (bvNumber != null) {
                        if (isBilibiliInstalled || isBilibiliHdInstalled) {
                            ...
                            if (isBilibiliHdInstalled) {
                                intent.setPackage(bilibiliHdPackageName)
                            } else {
                                intent.setPackage(bilibiliPackageName)
                            }
