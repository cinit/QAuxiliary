...
public class MsgRecordUtil {
     ...
     */
    @NotNull
    public static List<Integer> parse(@NotNull List<String> activeItems) {
        List<Integer> items = new ArrayList<>();
        for (String item : activeItems) {
            if (MSG.containsKey(item)) {
                items.add(MSG.get(item));
            }
        }
        return items;
    }
...
//            Toasts.info(Utils.getQQAppInterface().getApplication(), DESC.get(s));
     ...
     */
    @NotNull
    public static List<String> getKeys(@NotNull List<String> cache) {
     ...
     */
    @NotNull
    public static String getKey(@NotNull String cache) {
        BiMap<String, String> map = DESC.getReverseMap();
